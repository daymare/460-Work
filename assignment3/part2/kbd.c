...
// Event codes
// Event codes 0-100 are reserved for kbd events
#define EVENT_KBD_LINE 1
...
void kbd_handler()
{
    ...
    // wake up any processes sleeping on a kbd line
    if (c == '\r' || c == '\n')
    {
        printf("waking up processes waiting on event: %d!\n", EVENT_KBD_LINE);
        kwakeup(EVENT_KBD_LINE);
    }

    //printf("keyboard value: %c\n", c);
    ...
    printf("keyboard data: %d keyboard value: %c\n", kp->data, c);
}
...
void kgets(char* line)
{
    printf("sleeping on keyboard event: %d!\n", EVENT_KBD_LINE);
    ksleep(EVENT_KBD_LINE);
    printf("woke up from sleeping\n");
