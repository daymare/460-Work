...
int mkptable() // build level 1 page table using 1mb sections
{
    ...
    printf("1. build level-1 pagetable\n");
    ptable = (int*) LEVEL_ONE_PGTABLE_LOCATION;
    for (int i = 0; i < 4096; i++)
    {
        ptable[i] = 0;
    }

    printf("2. fill entries in level 1 page table with level 2 page tables\n");
    for (int i = 0; i < 512; i++)
    {
        ptable[i] = (0x500000 + i*1024) | 0x11; // domain = 0, cb = 00, type = 01
    }
    
    printf("3. build level 2 page tables\n");
    for (int i = 0; i < 512; i++)
    {
    ...
    printf("4. finished building two level page table\n");
}
